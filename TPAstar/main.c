//KHEYAR Adel et HALLAK Raphael  ING2 Grp 01
...
pSommet* CreerArete(pSommet* sommet, int s1, int s2, int poids)
{
    ...
    //création de l'arc de s2 vers s1
...
void dijkstra(Graphe* graphe, int sommet_depart, int sommet_arrive, int* sommets_explores_dijkstra)
{
    ...
    for (int i = 0; i < ordre; i++) {
        ...
        marque[min_sommet] = 1;
        (*sommets_explores_dijkstra)++;  //incrémenter lorsque le sommet est marqué comme explore
...
{
    int ordre = graphe->ordre;
    int distance[ordre], coutsommet[ordre];
    int marque[ordre];
    int predecesseur[ordre];

    for (int i = 0; i < ordre; i++) {
        distance[i] = INT_MAX;
        coutsommet[i] = INT_MAX;
        marque[i] = 0;
        predecesseur[i] = -1;
    }
    distance[sommet_depart] = 0;
    coutsommet[sommet_depart] = heuristique(graphe, sommet_depart, sommet_arrive);

    for (int i = 0; i < ordre; i++) {
        int mincout = INT_MAX;
        int min_sommet = -1;

        //trouver le sommet non marqué avec le plus petit coutsommet
        for (int j = 0; j < ordre; j++) {
            if (!marque[j] && coutsommet[j] < mincout) {
                mincout = coutsommet[j];
                min_sommet = j;
            }
        }
        ...
        while (arc != NULL) {
            ...
            if (!marque[successeur] && distance[min_sommet] + poids < distance[successeur]) {
                ...
                coutsommet[successeur] = distance[successeur] + heuristique(graphe, successeur, sommet_arrive);
...
int main()
{
    ...
    dijkstra(graphe, depart, arrivee, &sommets_explores_dijkstra);
    printf("Nombre de sommets explores avec Dijkstra: %d\n", sommets_explores_dijkstra);

    astar(graphe, depart, arrivee);
    printf("Nombre de sommets explores avec A*: %d\n", sommets_explores_astar);

    printf("On remarque qu'avec A* on obtient le meme chemin qu'avec dijikstra cepandant A* explore moins de sommet le rendant plus optimise.\n");
